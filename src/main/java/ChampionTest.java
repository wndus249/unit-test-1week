...
public class ChampionTest {
    ...
    @Test
    public void 부동소수점체크(){  //테스트 성공
        assertThat(3.14, closeTo(3, 0.2));  //여기서 0.2 -> 0.1로 바꾸면 에러
    }
    //의미없어 보이는 테스트일 수 있으나 에러가 발생하게 되면 테ㅡ트 실패
    //2가 아닌 5 이상의 값을 참조할 경우에는 에러 발생함
    @Test
    ...
    public void shouldHaveSomeChampName(){
        List<String> championNames = Arrays.asList("루시런", "애쉬", "럭사이", "갈리오", "모르가나", "블라다이르");
    ...
    public void shouldTopChampionIsDarius(){
        Optional<Champion> filteredChampion = championList.stream()
                ...
                .findFirst();
        String champName = (String)filteredChampion.get().getName();
